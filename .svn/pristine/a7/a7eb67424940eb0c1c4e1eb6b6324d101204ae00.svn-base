	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
			...
			if (child.getVisibility() != GONE) {
				...
				if (xpos + childw > width) {
					...
					for (int j = rowStartIdx; j < i; ++j) {
						...
						drawChild.layout(xpos, ypos,
								xpos + drawChild.getMeasuredWidth(), ypos
										+ drawChild.getMeasuredHeight());
						xpos += drawChild.getMeasuredWidth()
								+ ((LayoutParams) drawChild.getLayoutParams()).horizontal_spacing;
					}
		...
		if (rowStartIdx < count) {
			...
			for (int j = rowStartIdx; j < count; ++j) {
				...
				drawChild.layout(xpos, ypos,
						xpos + drawChild.getMeasuredWidth(),
						ypos + drawChild.getMeasuredHeight());
				xpos += drawChild.getMeasuredWidth()
						+ ((LayoutParams) drawChild.getLayoutParams()).horizontal_spacing;
			}
		}

...
public class ViewFlowEX extends AdapterView<Adapter> {
	...
	private float mLastMotionX, mLastX;
	...
	public ViewFlowEX(Context context) {
		super(context);
		mSideBuffer = 3;
		init();
	}

	public ViewFlowEX(Context context, int sideBuffer) {
		super(context);
	...
	public ViewFlowEX(Context context, AttributeSet attrs) {
		super(context, attrs);
			...
			if (v != null) {
				for (int index = 0; index < mAdapter.getCount(); index++) {
					if (v.equals(mAdapter.getItem(index))) {
						System.out.println(mAdapter.getItem(index));

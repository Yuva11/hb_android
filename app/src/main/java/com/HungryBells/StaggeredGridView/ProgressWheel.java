package com.HungryBells.StaggeredGridView;

/**
 * Created by Arunkumar on 06/12/2014.
 */
...
public class ProgressWheel extends View {
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressWheel_barWidthWheel,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_rimWidthWheel,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel_spinSpeedWheel,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressWheel_delayMillisWheel,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressWheel_barColorWheel, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressWheel_barLengthWheel,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressWheel_textSizeWheel,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressWheel_textColorWheel,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(R.styleable.ProgressWheel_textWheel)) {
            setText(a.getString(R.styleable.ProgressWheel_textWheel));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressWheel_rimColorWheel,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressWheel_circleColorWheel,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressWheel_contourColorWheel, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressWheel_contourSizeWheel, contourSize);
